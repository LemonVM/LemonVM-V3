...
impl GCObject{
    ...
    fn color_old(&mut self,color:TriColor,path:&mut Vec<*mut Self>){
        ...
        match self{
            YoungGCObject(young) => {
                panic!("ERROR! old generation object holds an young reference")
            },
            OldGCObject(old) => {
                match color{
                    TriColor::White => {
    ...
    fn mark_imm(&mut self,path:&mut Vec<*mut Self>){
        ...
        match self{
            ...
            _ => {
                panic!("ERROR! immortal generation object could only hold immortal generation reference")
            }
        }
    }
