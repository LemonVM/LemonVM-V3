...
pub fn interpreter(state: &mut VMState) {
    // 垃圾rust没GOTO,做个函数调用都她妈费劲
    'main_interpreter: loop {
        // init
        ...
        for r in 0..state.current_function_call_state.registers.len() {
            stack_regs[r] = state.current_function_call_state.registers[r].clone();
        }
        let mut pc = state.current_function_call_state.pc;
        if pc == 0 && state.args.len() > 0 {
            for i in 0..state.current_function_call_state.function_bytecode.args_count as usize {
        ...
        'fetch_exe: loop {
            // on exception
            if state.current_function_call_state.status == VMClosureStatus::Error {
                let mut find_exception_handle = false;
                // reset to error pc
                pc -= 1;
                if let Some(etable) = &state.current_function_call_state.function_bytecode.exception_table {
                    let handles = etable
                        .table
                        .iter()
                ...
                if !find_exception_handle {
                    ...
                    state
                        .exception_stack
                        // TODO: 看着优化了这个clone
                        .push(state.current_function_call_state.clone());
            ...
            if pc >= state.current_function_call_state.function_bytecode.code.len() as u16 {
                return;
                // TODO: I think that is an exception because a normal function at least has one return value or uses ret to return a void
            }
            let ci = state.current_function_call_state.function_bytecode.code[pc as usize];
            ...
            match ins {
                ...
                _ if ins == OpCode::SADD8 as u16 => {
                    ...
                    let v1 = match stack_regs[e1 as usize] {
                        ...
                        _ => {
                            panic!("ERROR! ARITH INSTRUCTION COULD ONLY APPLY TO NUMERICAL VALUES")
                        }
                    };
        ...
        // restore status
        state.current_function_call_state = state.current_function_call_state;
    }
}
